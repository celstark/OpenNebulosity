//==========================================================================
// API for the Opticstar DS-145M ICE.
// Version 1.00. 
...
// the camera model. iModel values: 0->DS-145M.
...
DLLDIR int OSDS145M_Initialize(int iModel, bool bOutVid, HWND hwOutVid, bool bStarView, int iRt);

// Always call this before exiting the application in order to clear the
// resources used by the camera.
DLLDIR int OSDS145M_Finalize();

// Shows or hides the StarView window.
DLLDIR int OSDS145M_ShowStarView(bool bStarView);
...
// the Windows window specified in OSDS145M_Initialize(). The iExposureTime is in 
...
DLLDIR int OSDS145M_VideoPreview(bool bPlay, int iBinningMode, int iExposureTime);
...
DLLDIR int OSDS145M_Capture(int iBinningMode, int iExposureTime);
...
DLLDIR int OSDS145M_IsExposing(bool *pbExposing, unsigned int *puiTimeRemaining);
...
// has been stopped.
DLLDIR int OSDS145M_StopExposure();
...
DLLDIR int OSDS145M_SetRawOffset(unsigned short wOffset);

// Gain settings. Range: 1 - 1023.
DLLDIR int OSDS145M_SetGain(unsigned short wGain);
DLLDIR int OSDS145M_GetGain(unsigned short *pwGain);

// Various image settings. Range: 0 - 255.
DLLDIR int OSDS145M_SetContrast(unsigned char byContrast);
DLLDIR int OSDS145M_GetContrast(unsigned char *pbyContrast);
DLLDIR int OSDS145M_SetBlackLevel(unsigned char byBlack);
DLLDIR int OSDS145M_GetBlackLevel(unsigned char *pbyBlack);
DLLDIR int OSDS145M_SetGamma(unsigned char byGamma);
DLLDIR int OSDS145M_GetGamma(unsigned char *pbyGamma);
DLLDIR int OSDS145M_SetSaturation(unsigned char bySaturation);
DLLDIR int OSDS145M_GetSaturation(unsigned char *pbySaturation);

// Auto Exposure Target value. Range: 0 - 255.
DLLDIR int OSDS145M_SetAeTarget(unsigned char byAeTarget);
DLLDIR int OSDS145M_GetAeTarget(unsigned char *pbyAeTarget);


// Gets a region of a captured native image as exported by the CCD camera to the 
// computer. It returns image data from the last OSDS145M_Capture() call. The 
...
// in OSDS145M_Capture() then all these parameters must be dividable by 2 and so on.
DLLDIR int OSDS145M_GetRawImage(int iX, int iY, int iWidth, int iHeight, void *pvRawBuf);

// Turns the raw data upside down.
DLLDIR int OSDS145M_InvertRawImage(int iWidth, int iHeight, void *pvRawBuf);

// Saves a RAW image to disk that has been retrieved by OSDS145M_GetRawImage().
...
//  (iWidth * iHeight * 2).
DLLDIR int OSDS145M_SaveRaw(char *paszName, int iWidth, int iHeight, void *pbyRawBuf);

// Converts a raw image returned by OSDS145M_GetRawImage() to a 24-bit RGB 
...
DLLDIR int OSDS145M_ConvertRawToRgb(int iWidth, int iHeight, void *pvRawBuf, void *pvRgbBuf);
...
DLLDIR int OSDS145M_ConvertRgbToBmp(int iWidth, int iHeight, void *pvRgbBuf, void *pvBmpBuf);

// Saves a 24-bit BMP image to disk including the ones created with OSDS145M_ConvertRgbToBmp().
DLLDIR int OSDS145M_SaveBmp(char *paszName, int iWidth, int iHeight, void *pbyBmpBuf);

// Converts a RGB image returned by OSDS145M_ConvertRawToRgb() to a JPG. 
// The RGB image buffer pvRgbBuf (source image) and the output image buffer 
// pvJpgBuf (destination image) must be provided by the application including 
// memory allocation and deallocation. The RGB buffer size should be 
// (iWidth * iHeight * 3). 
DLLDIR int OSDS145M_ConvertRgbToJpg(int iWidth, int iHeight, void *pvRgbBuf, void *pvJpgBuf);

// Saves a JPG image to disk including the ones created with OSDS145M_ConvertRgbToJpg().
DLLDIR int OSDS145M_SaveJpg(char *paszName, int iWidth, int iHeight, void *pbyJpgBuf);
