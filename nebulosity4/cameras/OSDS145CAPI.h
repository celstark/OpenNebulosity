//==========================================================================
// API for the Opticstar DS-145C ICE.
// Version 1.00. 
// Last updated: 15th January 2009.
// (c) Opticstar Ltd 2009. All rights reserved.
//==========================================================================
#ifdef DLLDIR_EX
...
// the camera model. iModel values: 0->DS-145M, 1->DS-145C.
...
DLLDIR int OSDS145C_Initialize(int iModel, bool bOutVid, HWND hwOutVid, bool bStarView, int iRt);

// Always call this before exiting the application in order to clear the
// resources used by the camera.
DLLDIR int OSDS145C_Finalize();

// Shows or hides the StarView window.
DLLDIR int OSDS145C_ShowStarView(bool bStarView);

// The OSDS145C_VideoPreview function is not currently supported.
...
// the Windows window specified in OSDS145C_Initialize(). The iExposureTime is in 
...
DLLDIR int OSDS145C_VideoPreview(bool bPlay, int iBinningMode, int iExposureTime);
...
//--------------------------------------------------------------------------------------
//  0     1x1     1360x1024  16-bit   ?      colour       yes      DS-145C
//  1     2x2      680x512   16-bit   ?      colour       yes      DS-145C
//  2     4x4      340x256   16-bit   ?      colour       yes      DS-145C
//  3     1x1     1360x1024   8-bit   ?      colour       yes      DS-145C
//  4     2x2      680x512    8-bit   ?      colour       yes      DS-145C
//  5     4x4      340x256    8-bit   ?      colour       yes      DS-145C
//
...
DLLDIR int OSDS145C_Capture(int iBinningMode, int iExposureTime);
...
DLLDIR int OSDS145C_IsExposing(bool *pbExposing, unsigned int *puiTimeRemaining);

// If an exposure takes place, it can be stopped by calling this function. 
// Keep calling OSDS145C_IsExposing() in order to determine whether the exposure 
// has been stopped.
DLLDIR int OSDS145C_StopExposure();
...
DLLDIR int OSDS145C_SetRawOffset(unsigned short wOffset);

// Gain settings in hardware. Range: 1 - 1023.
DLLDIR int OSDS145C_SetGain(unsigned short wGain);
DLLDIR int OSDS145C_GetGain(unsigned short *pwGain);

// Black Level settings in hardware. Range: 0 - 255.
DLLDIR int OSDS145C_SetBlackLevel(unsigned char byBlack);
DLLDIR int OSDS145C_GetBlackLevel(unsigned char *pbyBlack);

// Capture settings in software. Range: 0 - 255.
DLLDIR int OSDS145C_SetContrast(unsigned char byContrast);
DLLDIR int OSDS145C_GetContrast(unsigned char *pbyContrast);
DLLDIR int OSDS145C_SetGamma(unsigned char byGamma);
DLLDIR int OSDS145C_GetGamma(unsigned char *pbyGamma);
DLLDIR int OSDS145C_SetSaturation(unsigned char bySaturation);
DLLDIR int OSDS145C_GetSaturation(unsigned char *pbySaturation);

// Auto Exposure Target value. Range: 0 - 255.
DLLDIR int OSDS145C_SetAeTarget(unsigned char byAeTarget); 
DLLDIR int OSDS145C_GetAeTarget(unsigned char *pbyAeTarget);


// Gets a region of a captured native image as exported by the CCD camera to the 
// computer. It returns image data from the last OSDS145C_Capture() call. The 
...
// in OSDS145C_Capture() then all these parameters must be dividable by 2 and so on.
DLLDIR int OSDS145C_GetRawImage(int iX, int iY, int iWidth, int iHeight, void *pvRawBuf);

// Turns the raw data upside down.
DLLDIR int OSDS145C_InvertRawImage(int iWidth, int iHeight, void *pvRawBuf);

// Saves a RAW image to disk that has been retrieved by OSDS145C_GetRawImage().
...
//  (iWidth * iHeight * 2).
DLLDIR int OSDS145C_SaveRaw(char *paszName, int iWidth, int iHeight, void *pbyRawBuf);

// Saves the RAW data to disk in 24-bit BMP file format. The raw data must be retrieved 
// by OSDS145C_GetRawImage(). Loss of data is possible. To save the data intact call
// OSDS145C_SaveRaw().
DLLDIR int OSDS145C_SaveRawAsBmp(char *paszName, int iWidth, int iHeight, void *pbyRawBuf);

// Converts a raw image returned by OSDS145C_GetRawImage() to a 24-bit RGB 
...
DLLDIR int OSDS145C_ConvertRawToRgb(int iWidth, int iHeight, void *pvRawBuf, void *pvRgbBuf);

// Converts a RGB image returned by OSDS145C_ConvertRawToRgb() to a 24-bit BMP 
...
DLLDIR int OSDS145C_ConvertRgbToBmp(int iWidth, int iHeight, void *pvRgbBuf, void *pvBmpBuf);

// Saves a 24-bit BMP image to disk from the data returned by OSDS145C_ConvertRawToRgb().
DLLDIR int OSDS145C_SaveBmp(char *paszName, int iWidth, int iHeight, void *pbyBmpBuf);



// Converts a RGB image returned by OSDS145C_ConvertRawToRgb() to a JPG. 
// The RGB image buffer pvRgbBuf (source image) and the output image buffer 
// pvJpgBuf (destination image) must be provided by the application including 
// memory allocation and deallocation. The RGB buffer size should be 
// (iWidth * iHeight * 3). 
DLLDIR int OSDS145C_ConvertRgbToJpg(int iWidth, int iHeight, void *pvRgbBuf, void *pvJpgBuf);

// Saves a JPG image to disk including the ones created with OSDS145C_ConvertRgbToJpg().
DLLDIR int OSDS145C_SaveJpg(char *paszName, int iWidth, int iHeight, void *pbyJpgBuf);
