...
DLLDIR int OSDS615C_Initialize(int iModel, bool bOutVid, HWND hwOutVid, bool bStarViewMode, int iRt);

// Always call this before exiting the application in order to clear the
// resources used by the camera.
DLLDIR int OSDS615C_Finalize();

// Shows or hides the StarView window.
DLLDIR int OSDS615C_ShowStarView(bool bStarView);

// The OSDS615C_VideoPreview function is not currently supported.
DLLDIR int OSDS615C_VideoPreview(bool bPlay, int iBinningMode, int iExposureTime);
...
//                              depth     FPS  preserved?
//--------------------------------------------------------------------------------------
//  0     1x1     3032x2016     16-bit     ?    yes      for colour images
//  1     2x2     1516x1008     16-bit     ?    yes      for colour images
//  2     4x4      758x504      16-bit     ?    yes      for colour images
//  3     2x2     1516x1008     16-bit     ?    no       for monochrome images
//  4     4x4      758x504      16-bit     ?    no       for monochrome images
//  5     1x1     3032x2016      8-bit     ?    yes      for fast preview/focusing etc
//  6     2x2     1516x1008      8-bit     ?    no       for fast preview/focusing etc
//  7     4x4      758x504       8-bit     ?    no       for fast preview/focusing etc
//
// The exposure time, lExpTime, must be in milliseconds.
DLLDIR int OSDS615C_Capture(int iBinningMode, int iExposureTime);
...
DLLDIR int OSDS615C_IsExposing(bool *pbExposing, unsigned int *puiTimeRemaining);
...
// has been stopped.
DLLDIR int OSDS615C_StopExposure();
...
DLLDIR int OSDS615C_GetRawImage(int iX, int iY, int iWidth, int iHeight, void *pvRawBuf);
...
DLLDIR int OSDS615C_SaveRaw(char *paszName, int lWidth, int lHeight, void *pbyRawBuf);
...
DLLDIR int OSDS615C_ConvertRawToRgb(int iWidth, int iHeight, void *pvRawBuf, void *pvRgbBuf);
...
DLLDIR int OSDS615C_InvertRgb(int iWidth, int iHeight, void *pvRgbBuf);

// It reflects the RGB data so that left becomes right and vice versa.
// Typically, the RGB data used has been returnrd by OSDS615C_ConvertRawToRgb().
// The buffer's memory allocation must be provided by the application.
DLLDIR int OSDS615C_ReflectRgb(int iWidth, int iHeight, void *pvRgbBuf);
...
DLLDIR int OSDS615C_ConvertRgbToBmp(int iWidth, int iHeight, void *pvRgbBuf, void *pvBmpBuf);

// Saves a 24-bit BMP image to disk from the data returned by OSDS615C_ConvertRawToRgb().
DLLDIR int OSDS615C_SaveBmp(char *paszName, int iWidth, int iHeight, void *pbyBmpBuf);
...
// memory allocation and deallocation. The RGB buffer size should be 
// (iWidth * iHeight * 3). 
DLLDIR int OSDS615C_ConvertRgbToJpg(int iWidth, int iHeight, void *pvRgbBuf, void *pvJpgBuf);

// Saves a JPG image to disk including the ones created with OSDS615C_ConvertRgbToJpg().
DLLDIR int OSDS615C_SaveJpg(char *paszName, int iWidth, int iHeight, void *pbyJpgBuf);

//-------------------------------------------------------------------------------------
// The following functions are performed in hardware.

// Gain, for manual exposure only. Range: 1 - 1023. Default value is 1.
DLLDIR int OSDS615C_SetGain(unsigned short  wGain);
DLLDIR int OSDS615C_GetGain(unsigned short *pwGain);

// Back Level. Range: 1 - 255. Default value is 8.
DLLDIR int OSDS615C_SetBlackLevel(unsigned char byBlack);
DLLDIR int OSDS615C_GetBlackLevel(unsigned char *pbyBlack);
...
DLLDIR int OSDS615C_SetDataOffset(unsigned short wOffset);

// Sets Automatic White Balance to true or false.
DLLDIR int OSDS615C_SetAWB(bool bAwb);

// Sets saturation level between 0 - 255.
DLLDIR int OSDS615C_SetSaturation(unsigned char bySaturation);

// Sets contrast level between 0 - 255.
DLLDIR int OSDS615C_SetContrast(unsigned char byContrast);

// Sets gamma level between 0 - 3.98.
DLLDIR int OSDS615C_SetGamma(float flGamma);

// Sets RGB gain levels between 0 - 3.98.
DLLDIR int OSDS615C_SetColorGain(float flRed, float flGreen, float flBlue);
...
DLLDIR int OSDS615C_Set8bitRolling(bool b8bitRolling);
DLLDIR int OSDS615C_Get8bitRolling(bool *pb8bitRolling);
DLLDIR int OSDS615C_Set16bitRolling(bool b16bitRolling);
DLLDIR int OSDS615C_Get16bitRolling(bool *pb16bitRolling);
